...
**Convert & export**
- **PDF → Markdown** — tagged-PDF structure when available, PDFium heuristic layout otherwise; on save, scanned/sparse pages, embedded images, and painted Form XObjects (vector charts rasterized via PDFium) land in `<md-stem>_assets/` with optional Tesseract OCR; auto-save sibling `.md` or **Save As…** for a custom path
