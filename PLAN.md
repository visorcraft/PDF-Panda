...
| Metadata | View/edit Info dictionary (title, author, subject, keywords, creator, producer); ModDate updated on save | `get_pdf_metadata`, `set_pdf_metadata`, `set_pdf_metadata_roundtrip`, UI validation |
...
- `cargo test` — 139 unit tests (+ 3 ignored: `render_real_pdf_smoke`, `export_e2e_sample_pdf`, `ocr_rendered_page_smoke`) covering
...
- [x] **Document metadata** — view/edit PDF Info fields (`get_pdf_metadata`, `set_pdf_metadata`, Metadata modal)
