      ...
      <div className="credits-table-body" role="list">
        {rows.map((row, index) => (
          ...
          >
            ...
            <span className="credits-col-version credits-mono">{row.version}</span>
...
export function CreditsModal({ onClose }: { onClose: () => void }) {
  ...
  const filterRows = useCallback((rows: ThirdPartyCreditRow[], needle: string) => {
  ...
  const openComponentLicense = useCallback(async (component: RuntimeComponentRow) => {
    ...
    for (const spdx of component.spdx) {
      const body = await invoke<string>('runtime_license_text', { spdxId: spdx });
      if (body.trim()) {
        sections.push(`===== ${spdx} =====\n\n${body}`);
      }
    }
              ...
              <>
                <section className="credits-runtime-card">
                  ...
                  <p className="credits-runtime-help">
                    System libraries and bundled runtimes PDF-Panda uses at execution time.
                    Packaged builds ship PDFium; Linux desktop builds use the system WebKitGTK
                    and GTK stacks. Tesseract is optional for scan OCR.
                  </p>
                  <ul className="credits-runtime-list">
                    {catalog.runtime_components.map((component) => (
                      <li key={component.name} className="credits-runtime-row">
                        <span className="credits-runtime-name">{component.name}</span>
                        <span className="credits-runtime-license">{component.licenses}</span>
                ...
                <CreditsTable
                  rows={filteredCrates}
                  emptyLabel={crateFilter.trim() ? 'No Cargo crates match the current filter.' : 'No Cargo crates listed.'}
                  onOpenUrl={openUrl}
                />
                ...
                <CreditsTable
                  rows={filteredNpm}
                  emptyLabel={npmFilter.trim() ? 'No npm packages match the current filter.' : 'No npm packages listed.'}
                  onOpenUrl={openUrl}
                />
              </>
            )}
