...
def patch_main(main_text: str, handlers: list[str], tests: list[str]) -> str:
    inc_marker = "// PARITY_BATCH7_INCLUDE"
    ...
    hstart = "// PARITY_BATCH7_HANDLERS_START"
    hend = "// PARITY_BATCH7_HANDLERS_END"
    block = f"            {hstart}\n" + "\n".join(handlers) + f"\n            {hend}"
    ...
    tstart = "// PARITY_BATCH7_TESTS_START"
    tend = "// PARITY_BATCH7_TESTS_END"
    test_block = f"    {tstart}\n" + "\n".join(tests) + f"\n    {tend}"
