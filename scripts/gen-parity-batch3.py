#!/usr/bin/env python3
"""Generate batch 36–45: 66 mod-3 extended + 34 half-range-in-range commands."""
...
INC3 = ROOT / "src-tauri" / "src" / "parity_batch3_generated.inc.rs"
...
MOD3_EXTENDED = [
    ("duplicate_pages_in_range_before_mod3", "dup_before"),
    ("duplicate_pages_in_range_to_start_mod3", "dup_to_start"),
    ("duplicate_pages_in_range_to_end_mod3", "dup_to_end"),
    ("expand_pages_in_range_mod3", "expand"),
    ("shrink_pages_in_range_mod3", "shrink"),
    ("clear_crop_pages_in_range_mod3", "clear_crop"),
    ("insert_blank_before_pages_in_range_mod3", "blank_before"),
    ("insert_blank_after_pages_in_range_mod3", "blank_after"),
    ("bookmark_pages_in_range_mod3", "bookmark"),
    ("set_page_size_pages_in_range_mod3", "page_size"),
    ("add_page_numbers_pages_in_range_mod3", "page_numbers"),
    ("add_text_watermark_pages_in_range_mod3", "watermark"),
    ("add_page_header_pages_in_range_mod3", "header"),
    ("add_page_footer_pages_in_range_mod3", "footer"),
    ("add_page_border_pages_in_range_mod3", "border"),
    ("export_pages_in_range_mod3_webp", "export_webp"),
    ("export_pages_in_range_mod3_bmp", "export_bmp"),
    ("export_pages_in_range_mod3_tiff", "export_tiff"),
    ("export_pages_in_range_mod3_gif", "export_gif"),
    ("export_pages_in_range_mod3_ppm", "export_ppm"),
    ("export_pages_in_range_mod3_tga", "export_tga"),
    ("export_pages_in_range_mod3_ico", "export_ico"),
]

MOD3_SINGLE = [
    ("export_pages_in_range_mod3_jpeg", "export_jpeg", 1),
    ("export_pages_in_range_mod3_jpeg", "export_jpeg", 2),
]
...
HALF_HELPERS = [
    "parity_half_indices",
    "parity_half_rotate",
    "parity_half_reset_rotation",
    "parity_half_delete",
    "parity_half_keep",
    "parity_half_dup_append",
    "parity_half_flatten",
    "parity_half_reverse",
    "parity_half_crop",
    "parity_half_extract",
    "parity_half_export_pdf",
    "parity_half_export_rendered",
    "parity_half_page_numbers",
    "parity_half_watermark",
]
...
def mod3_supplemental_helpers() -> str:
    ...
    chunk = chunk.replace(
        "parity_mod3_match(idx, start_page, end_page, keep_odd, local)",
        "parity_mod3_match(idx, start_page, end_page, remainder)",
    )
    kept: list[str] = []
    for name in MOD3_SUPPLEMENTAL:
        m = re.search(
...
def mod3_impl_call(kind: str, remainder: int) -> str:
    ...
    pl = f"{rng}, {r}"
    calls = {
        "dup_before": f"parity_mod3_dup_before({pl})",
        "dup_to_start": f"parity_mod3_dup_to_start({pl})",
        "dup_to_end": f"parity_mod3_dup_to_end({pl})",
        "expand": f"parity_mod3_expand({pl}, margin_top, margin_right, margin_bottom, margin_left)",
        "shrink": f"parity_mod3_shrink({pl}, margin_top, margin_right, margin_bottom, margin_left)",
        "clear_crop": f"parity_mod3_clear_crop({pl})",
        "blank_before": f"parity_mod3_blank({pl}, false)",
        "blank_after": f"parity_mod3_blank({pl}, true)",
        "bookmark": f"parity_mod3_bookmark({pl}, prefix)",
        "page_size": f"parity_mod3_page_size({pl}, &preset)",
        "page_numbers": f"parity_mod3_page_numbers({pl}, prefix)",
        "watermark": f"parity_mod3_watermark({pl}, &text)",
        "header": f"parity_mod3_header({pl}, &text)",
        "footer": f"parity_mod3_footer({pl}, &text)",
        "border": f"parity_mod3_border({pl}, inset)",
        "export_webp": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "webp", render_page_webp)',
        "export_bmp": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "bmp", render_page_bmp)',
        "export_tiff": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "tiff", render_page_tiff)',
        "export_gif": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "gif", render_page_gif)',
        "export_ppm": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "ppm", render_page_ppm)',
        "export_tga": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "tga", render_page_tga)',
        "export_ico": f'parity_mod3_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {r}, "ico", render_page_ico)',
...
def half_impl_call(kind: str, first_half: bool) -> str:
    ...
    pl = f"{rng}, {fh}"
    calls = {
        "rotate_cw": f"parity_half_rotate({pl}, 90)",
        "rotate_ccw": f"parity_half_rotate({pl}, -90)",
        "rotate_180": f"parity_half_rotate({pl}, 180)",
        "reset_rot": f"parity_half_reset_rotation({pl})",
        "delete": f"parity_half_delete({pl})",
        "keep": f"parity_half_keep({pl})",
        "dup_append": f"parity_half_dup_append({pl})",
        "flatten": f"parity_half_flatten({pl})",
        "reverse": f"parity_half_reverse({pl})",
        "crop": f"parity_half_crop({pl}, margin_top, margin_right, margin_bottom, margin_left)",
        "extract": f"parity_half_extract({path}, &PathBuf::from(&output_path), start_page, end_page, {fh})",
        "export_pdf": f"parity_half_export_pdf({path}, &PathBuf::from(&output_dir), start_page, end_page, {fh})",
        "export_png": f'parity_half_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {fh}, "png", render_page_png)',
        "export_jpeg": f'parity_half_export_rendered({path}, &PathBuf::from(&output_dir), start_page, end_page, {fh}, "jpeg", render_page_jpeg)',
        "page_numbers": f"parity_half_page_numbers({pl}, prefix)",
        "watermark": f"parity_half_watermark({pl}, &text)",
    }
    return calls[kind]
...
def patch_main(main_text: str, handlers: list[str], tests: list[str]) -> str:
    inc_marker = "// PARITY_BATCH3_INCLUDE"
    ...
    hstart = "// PARITY_BATCH3_HANDLERS_START"
    hend = "// PARITY_BATCH3_HANDLERS_END"
    block = f"            {hstart}\n" + "\n".join(handlers) + f"\n            {hend}"
    ...
    tstart = "// PARITY_BATCH3_TESTS_START"
    tend = "// PARITY_BATCH3_TESTS_END"
    test_block = f"    {tstart}\n" + "\n".join(tests) + f"\n    {tend}"
...
def load_prior_command_names() -> list[str]:
    ...
    names: list[str] = []
    for script in ("gen-parity-batch.py", "gen-parity-batch2.py"):
...
def main() -> None:
    ...
    lines = [
        "// Auto-generated by scripts/gen-parity-batch3.py — do not edit.",
        mod3_supplemental_helpers(),
        half_rust_helpers(),
    ]
    tests = ["// Auto-generated parity batch3 tests"]
    handlers: list[str] = []
    ...
    INC3.write_text("\n".join(lines))
    print(f"Wrote {INC3} ({len(specs)} commands)")
